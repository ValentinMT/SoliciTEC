...
<html>
  ...
  <?php 
      $nom=\DB::table('empleado as E')
            ...
            ->join('solicitudes as S', 'S.claveEmp', '=', 'E.clave')
            ->where('S.folio', '=', $id)
            ->select('D.nombre')
            ->first();
      ...
      $solicitudes=\DB::table('solicitudes as S')
            ...
            ->select('S.folio', 'S.fechaCaptura','E.nombre as nombreEmp', 'S.problema', 
              'S.urgencia', 'D.nombre as nombreDptoDestino', 'E.departamento_clave',
              'Q.descripcion', 'Q.imagen')
            ->where('S.folio', '=', $id)
            ->first();
   ?>  
    <table id="two" style="width:100%">
      ...
      <tr>
        <td>{{$solicitudes->fechaCaptura}}</td>
        <td></td>
        <td>{{$solicitudes->folio}}</td>
      </tr>
      <tr>
        <td colspan="3"><b>Nombre del Empleado</b></td>
      </tr>
      <tr>
        <td colspan="3">{{$solicitudes->nombreEmp}}</td>
      </tr>
      <tr>
        <td><b>Problema</b></td>
        <td></td>
      ...
      <tr>
        ...
        ?>
        <?php 
          if($solicitudes->urgencia=='1') {
            $var1='Baja';
          } else if($solicitudes->urgencia=='2') {
            $var1=' Media';
          } else {
            $var1='Alta';
          }
      ...
      <tr>
          <td>{{$nom->nombre}}</td>
          <td></td>
          <td>{{$solicitudes->nombreDptoDestino}}</td>
      </tr>
    </table>
...
<!--Pagina 2////////////////////////////////////////////////-->
    ...
    <table class="seven3" style="width:100%">
      <tr>
        <td align=center>
          Información de quejas relacionadas con la solicitud #{{$solicitudes->folio}}
        </td>
      </tr>
    </table>
      <?php
         $anexos=\DB::table('esAtendido')
              ...
              ->where('esAtendido.solicitudes_folio','=',$id)
